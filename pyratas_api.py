...
def require_license_table(conn: sqlite3.Connection):
    if not table_exists(conn, "license"):
        # banco subiu sem o arquivo licenses.db versionado ou caminho errado
...
def _startup():
    """
    Inicializa a conexão global e garante a tabela de activations.
    NÃO cria a tabela 'license' — ela deve vir do seu arquivo licenses.db versionado.
    """
    conn = _connect_once()
